...
import type { APGamesInformation, GameBase, GameBaseSimultaneous } from "@abstractplay/gameslib";
...
export class Game {
	...
	constructor(opts: { game: GameState }) {
		...
		if (this._gameState.data !== undefined) {
			...
			this._engine = GameFactory(this._gameState.data.metaGame, this._gameState.data.state);
		}
	}
